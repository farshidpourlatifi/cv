...
// Curated, human-facing skill groups selected in cv-data.json (the editorial
// choice lives in the data, not the component). Optional with a safe default so
// the site renders even before the exporter emits it.
export const FeaturedSkillGroupSchema = z.object({
  label: z.string().min(1),
  items: z.array(z.string()),
});

export const SkillsSchema = z.object({
  featured: z.array(FeaturedSkillGroupSchema).default([]),
...
export const SiteConfigSchema = z.object({
  ...
  // Social-share title, longer/more descriptive than the tab <title>.
  // Optional: falls back to `title` when the export omits it.
...
export type FeaturedSkillGroup = z.infer<typeof FeaturedSkillGroupSchema>;
